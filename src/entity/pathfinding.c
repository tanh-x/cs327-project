...
int **generateDistanceField(Map *map, int sourceX, int sourceY, EntityType entityType) {
    ...
    if (distanceField == NULL) return NULL;
    ...
    // Now, distanceField is a 2D array filled with the same value (`UNCROSSABLE`)
    ...
    if (source->cost == UNCROSSABLE) return distanceField;
...
void printDistanceField(int **distanceField) {
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            if (distanceField[y][x] == UNVISITED) printf("   ");
            else printf("%02d ", distanceField[y][x] % 100);
        }
        printf("\n");
    }
}
...
void printDistanceFieldAlt(int **distanceField) {
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            if (distanceField[y][x] == UNVISITED) printf("   ");
            else printf("%02d ", (distanceField[y][x] / 10) % 100);
        }
        printf("\n");
    }
}
